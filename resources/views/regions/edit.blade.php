...
<li class="breadcrumb-item active" aria-current="page">{{ __('Edit Region') }}</li>
@endcomponent
@endcomponent
    ...
    <div class="row">
        <div class="col-xl-12 order-xl-1">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="post" action="{{ route('region.update', $region) }}" autocomplete="off" enctype="multipart/form-data">
                        @csrf
                        @method('put')
                        ...
                        <div class="pl-lg-4">
                            <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                ...
                                <input type="text" name="name" id="input-name" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="{{ __('Name') }}" value="{{ old('name', $region->name) }}" required autofocus>
                            ...
                            <div class="form-group{{ $errors->has('account_id') ? ' has-danger' : '' }}">
                                ...
                                <select name="account_id" id="input-role" class="form-control{{ $errors->has('account_id') ? ' is-invalid' : '' }}" placeholder="{{ __('Account') }}" required>
                                    ...
                                    <option value="{{ $account->id }}" {{ $account->id == old('account_id', $region->account_id) ? 'selected' : '' }}>{{ $account->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group{{ $errors->has('lat') ? ' has-danger' : '' }}">
                                ...
                                <input type="text" name="lat" id="input-name" class="form-control{{ $errors->has('lat') ? ' is-invalid' : '' }}" placeholder="{{ __('Latitude') }}" value="{{ old('lat', $region->lat) }}" required autofocus>
                            ...
                            <div class="form-group{{ $errors->has('lon') ? ' has-danger' : '' }}">
                                ...
                                <input type="text" name="lon" id="input-name" class="form-control{{ $errors->has('lon') ? ' is-invalid' : '' }}" placeholder="{{ __('Longitude') }}" value="{{ old('lon', $region->lon) }}" required autofocus>
